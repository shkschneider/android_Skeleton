...
object Notify {
    ...
    fun toast(msg: String, duration: Int = Toast.LENGTH_LONG) {
        ...
        Toast.makeText(ContextProvider.applicationContext(), msg, duration)?.apply {
            show()
        }
    }
    ...
    fun chip(@ColorRes color: Int, text: String, @DrawableRes drawable: Int? = null): Chip =
            Chip(ContextProvider.applicationContext()).apply {
                setText(text)
                if (drawable != null) {
...
fun Snackbar.material() {
    ...
    view.background = ContextCompat.getDrawable(ContextProvider.applicationContext(), R.drawable.sk_snackbar)
    ViewCompat.setElevation(view, ContextProvider.applicationContext().resources.getDimension(R.dimen.sk_elevation6))
}
